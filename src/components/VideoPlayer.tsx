...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ sources, activeSourceIndex, onSwitchSource, poster }) => {
  ...
  const handleFullscreenLogic = async () => {
    ...
    if (player.isFullscreen()) {
      try {
        // Minta rotasi layar ke landscape saat fullscreen
        // @ts-ignore
      ...
      // Lepas lock saat keluar fullscreen
      // @ts-ignore
    ...
    const player = playerRef.current = videojs(videoElement, {
      ...
      controlBar: {
        children: [
          ...
          'subsCapsButton',
          'audioTrackButton',
          'fullscreenToggle', // Fullscreen button tetap ada
        ],
      },
    });
  ...
  return (
    <div className="w-full h-full relative overflow-hidden rounded-2xl bg-black group">
      ...
      {/* INI OVERLAY UNTUK SECURITY TAPI TETAP BISA CLICK PLAYER */}
      ...
      <style jsx global>{`
        ...
        /* Blokir interaksi ke iframe Youtube langsung */
        .vjs-youtube .vjs-tech { pointer-events: none !important; }
        
        /* Pastikan control bar VideoJS ada di paling atas (Z-INDEX) agar bisa dipencet */
        .vjs-control-bar { z-index: 30 !important; }

        .vjs-quality-menu-button { width: 40px !important; display: flex !important; align-items: center; justify-content: center; cursor: pointer; }
        .vjs-quality-icon-container svg { width: 18px; height: 18px; }
        .vjs-menu-content { background: rgba(0,0,0,0.95) !important; border-radius: 8px; bottom: 4em !important; }
